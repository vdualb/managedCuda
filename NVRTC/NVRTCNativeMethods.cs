...
namespace ManagedCuda.NVRTC
{
    ...
    {
        internal const string NVRTC_API_DLL_NAME = "nvrtc64_130_0";

#if (NETCOREAPP)
        internal const string NVRTC_API_DLL_NAME_LINUX = "nvrtc";
        internal const string NVRTC_SHORT_VERSION_LINUX = "13";
        internal const string NVRTC_LONG_VERSION_LINUX = "13.0.88";
        ...
        private static IntPtr ImportResolver(string libraryName, System.Reflection.Assembly assembly, DllImportSearchPath? searchPath)
        {
            ...
            if (libraryName == NVRTC_API_DLL_NAME)
            {
                if (RuntimeInformation.IsOSPlatform(OSPlatform.Linux))
                {
                    ...
                    bool res = NativeLibrary.TryLoad("lib" + NVRTC_API_DLL_NAME_LINUX + ".so." + NVRTC_LONG_VERSION_LINUX, assembly, DllImportSearchPath.SafeDirectories, out libHandle);
                    if (res)
                    {
                        return libHandle;
                    }
                    ...
                    res = NativeLibrary.TryLoad("lib" + NVRTC_API_DLL_NAME_LINUX + ".so." + NVRTC_SHORT_VERSION_LINUX, assembly, DllImportSearchPath.SafeDirectories, out libHandle);
                    if (res)
                    {
                        return libHandle;
                    }
                    ...
                    res = NativeLibrary.TryLoad(NVRTC_API_DLL_NAME_LINUX, assembly, DllImportSearchPath.SafeDirectories, out libHandle);
                    if (!res)
                    {
